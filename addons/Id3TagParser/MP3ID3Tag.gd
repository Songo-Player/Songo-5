...
func _getFrameImage(frameName: StringName) -> Dictionary:
	var data := _getFrameBytes(frameName)
	if data.is_empty():
		return {}
	...
	# 1️⃣ Text encoding byte
	var encoding := buff.get_u8()
	var is_utf16 := encoding in [1, 2]
	var is_utf8 := encoding == 3

	# 2️⃣ MIME type (always ASCII, null-terminated)
	var mime_bytes := PackedByteArray()
	while buff.get_position() < buff.data_array.size():
		var b := buff.get_u8()
		if b == 0:
			break
		mime_bytes.append(b)
	var mime_type := mime_bytes.get_string_from_ascii()

	# 3️⃣ Picture type (1 byte)
	var pic_type := buff.get_u8()

	# 4️⃣ Description (depends on encoding)
	var descr_bytes := PackedByteArray()
	while buff.get_position() < buff.data_array.size():
		var b := buff.get_u8()
		descr_bytes.append(b)
		if is_utf16:
			if descr_bytes.size() >= 2 and descr_bytes[-1] == 0 and descr_bytes[-2] == 0:
				break
		elif b == 0:
			break
	...
	if is_utf16:
		# remove terminator
		if descr_bytes.size() >= 2 and descr_bytes[-1] == 0 and descr_bytes[-2] == 0:
			descr_bytes = descr_bytes.slice(0, descr_bytes.size() - 2)
		description = descr_bytes.get_string_from_utf16()
	elif is_utf8:
		if descr_bytes.size() > 0 and descr_bytes[-1] == 0:
			descr_bytes = descr_bytes.slice(0, descr_bytes.size() - 1)
		description = descr_bytes.get_string_from_utf8()
	else:
		if descr_bytes.size() > 0 and descr_bytes[-1] == 0:
			descr_bytes = descr_bytes.slice(0, descr_bytes.size() - 1)
		description = descr_bytes.get_string_from_ascii()

	# 5️⃣ Remaining bytes = image data
	var img_bytes := buff.data_array.slice(buff.get_position())

	return {
		"mimeType": mime_type,
		"pictureType": pic_type,
		"description": description,
		"pictureBytes": img_bytes
	}
