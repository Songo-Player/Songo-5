...
func _decode_head() -> ID3MainHeader:
	var header_bytes := _stream.data.slice(0, TAG_HEADER_LENGTH)
...
func _decode_frame_heads() -> Dictionary:
	...
	while frame_start < _ID3Header.size:
		var frame_header_bytes := _stream.data.slice(frame_start, frame_start + FRAME_HEADER_LENGTH)
...
func getFrameData(frameName: StringName) -> Variant:
	...
	match frameName:
		...
		"APIC":
			return null # Didnt work anyways
		_:
			return null

func _getFrameBytes(frameName: StringName) -> PackedByteArray:
	var start = frames[frameName][0] + FRAME_HEADER_LENGTH
	var end = start + frames[frameName][1]
...
# -------------------------------
# Text Decoding (Encoding Handling)
# -------------------------------
...
func _decodeByteText(text: PackedByteArray, isUnicode: bool) -> String:
	...
	if isUnicode:
		...
		# Default fallback to UTF-8 if no BOM
		return text.get_string_from_utf8()
	else:
		# Handle ISO-8859-1 on Linux (where ASCII may fail)
		var s := text.get_string_from_utf8()
		if not s.is_valid_identifier():  # crude heuristic; replace if garbled
