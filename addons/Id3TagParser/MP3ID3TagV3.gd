...
# Fast but innacurate
func getMp3LengthSeconds() -> float:
	var file = FileAccess.open(file_path, FileAccess.READ)
	if not file:
		return 0.0

	# Skip ID3v2 tag
	var header_bytes = file.get_buffer(10)
	if header_bytes[0] == 0x49 and header_bytes[1] == 0x44 and header_bytes[2] == 0x33:
		var size = ((header_bytes[6] & 0x7F) << 21) | ((header_bytes[7] & 0x7F) << 14) \
				 | ((header_bytes[8] & 0x7F) << 7) | (header_bytes[9] & 0x7F)
		file.seek(10 + size)
	else:
		file.seek(0)

	# Read first audio frame header
	var frame_header = file.get_buffer(4)
	if frame_header.size() < 4:
		file.close()
		return 0.0

	var bitrate_index = (frame_header[2] >> 4) & 0x0F
	var sample_rate_index = (frame_header[2] >> 2) & 0x03

	var bitrates = [0,32,40,48,56,64,80,96,112,128,160,192,224,256,320] # kbps for MPEG1 Layer3
	var sample_rates = [44100, 48000, 32000] # Hz

	var bitrate = bitrates[bitrate_index] * 1000
	var sample_rate = sample_rates[sample_rate_index]

	var file_size_bits = (file.get_length() - file.get_position()) * 8
	file.close()

	if bitrate > 0:
		return float(file_size_bits) / float(bitrate)
	return 0.0
